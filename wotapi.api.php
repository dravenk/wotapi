...
/**
 * @file
 * Documentation related to WOT:API.
 */

use Drupal\Core\Access\AccessResult;

/**
 * @defgroup wotapi_architecture WOT:API Architecture
 * @{
 *
 * @section overview Overview
 * The WOT:API module is a Drupal-centric implementation of the WOT:API
 * specification. By its own definition, the WOT:API specification "is a
 ...
 *
 * While "Drupal-centric", the WOT:API module is committed to strict compliance
 ...
 * these resources.
 *
 * The WOT:API module maps every entity type + bundle to a resource type.
 * Since the specification does not have a concept of resource type inheritance
 * or composition, the WOT:API module implements different bundles of the same
 * entity type as *distinct* resource types.
 *
 * While it is theoretically possible to expose arbitrary data as resources, the
 * WOT:API module only exposes resources from (config and content) entities.
 ...
 * The WOT:API module exposes entity revisions in a manner inspired by RFC5829:
 * Link Relation Types for Simple Version Navigation between Web Resources.
 *
 * Revision support is not an official part of the WOT:API specification.
 * However, a number of "profiles" are being developed (also not officially part
 * in the spec, but already committed to WOT:API v1.1) to standardize any
 * custom behaviors that the WOT:API module has developed (all of which are
 ...
 * @see https://www.drupal.org/project/wotapi/issues/2955020
 *
 * By implementing revision support as a profile, the WOT:API module should be
 * maximally compatible with other systems.
 *
 * A "version" in the WOT:API module is any revision that was previously, or is
 ...
 *
 * Using WOT:API, one could request the "Published" node by requesting
 * `/wotapi/node/page/{{uuid}}?resourceVersion=rel:latest-version`.
 *
 * To preview an entity that is still a work-in-progress (i.e. the "Draft"
 * revision) one could request
 * `/wotapi/node/page/{{uuid}}?resourceVersion=rel:working-copy`.
 *
 * To request a specific revision ID, one can request
 * `/wotapi/node/page/{{uuid}}?resourceVersion=id:{{revision_id}}`.
 *
 * It is not yet possible to request a collection of revisions. This is still
 * under development in issue [#3009588].
 *
 * @see https://www.drupal.org/project/wotapi/issues/3009588.
 * @see https://tools.ietf.org/html/rfc5829
 * @see https://www.drupal.org/docs/8/modules/wotapi/revisions
 *
 * @section translations Resource translations
 *
 * Some multilingual features currently do not work well with WOT:API. See
 * WOT:API modules's multilingual support documentation online for more
 * information on the current status of multilingual support.
 *
 * @see https://www.drupal.org/docs/8/modules/wotapi/translations
 *
 * @section api API
 * The WOT:API module provides an HTTP API that adheres to the WOT:API
 * specification.
 *
 * The WOT:API module provides *no PHP API to modify its behavior.* It is
 ...
 *   WOT:API module, the data must be defined as entity. See the "Resources"
 *   section.
 ...
 *   REST modules are also used by the WOT:API module.
 * - All available authentication mechanisms are allowed.
 *
 * @section tests Test Coverage
 * The WOT:API module comes with extensive unit and kernel tests. But most
 ...
 * \Drupal\Tests\wotapi\Functional\ResourceTestBase, which is a base class
 ...
 * \Drupal\Tests\wotapi\Functional\TestCoverageTest.
 *
 ...
 * the design of the Symfony Serialization component, not because the WOT:API
 * module wanted to publicly expose services.
 *
 * HTTP API: URLs and JSON response structures are considered part of this
 * module's public API. However, inconsistencies with the WOT:API specification
 ...
 * the WOT:API specification used under its "wotapi" key. Future releases
 ...
 * released.
 *
 * @see http://wotapi.org/faq/#what-is-the-meaning-of-json-apis-version
 *
 * Tests: subclasses of base test classes may contain BC breaks between minor
 * releases, to allow minor releases to A) comply better with the WOT:API spec,
 * B) guarantee that all resource types (and therefore entity types) function as
 * expected, C) update to future versions of the WOT:API spec.
 *
 * @}
 */
...
/**
 * Controls access when filtering by entity data via WOT:API.
 *
 ...
 * @code
 * /wotapi/node/article?filter[uid.field_favorite_animal]=llama
 * @endcode
 ...
 * cannot be bypassed via a WOT:API filtered query.
 *
 ...
 * - Implement this hook (or hook_wotapi_ENTITY_TYPE_filter_access()) and
 ...
 *   - WOTAPI_FILTER_AMONG_ALL: all entities of the given type.
 *   - WOTAPI_FILTER_AMONG_PUBLISHED: all published entities of the given type.
 *   - WOTAPI_FILTER_AMONG_ENABLED: all enabled entities of the given type.
 *   - WOTAPI_FILTER_AMONG_OWN: all entities of the given type owned by the
 ...
 */
function hook_wotapi_entity_filter_access(\Drupal\Core\Entity\EntityTypeInterface $entity_type, \Drupal\Core\Session\AccountInterface $account) {
...
/**
 * Controls access to filtering by entity data via WOT:API.
 *
 * This is the entity-type-specific variant of
 * hook_wotapi_entity_filter_access(). For implementations with logic that is
 * specific to a single entity type, it is recommended to implement this hook
 * rather than the generic hook_wotapi_entity_filter_access() hook, which is
 ...
 *   hook_wotapi_entity_filter_access() for details.
 *
 * @see hook_wotapi_entity_filter_access()
 */
function hook_wotapi_ENTITY_TYPE_filter_access(\Drupal\Core\Entity\EntityTypeInterface $entity_type, \Drupal\Core\Session\AccountInterface $account) {
  return ([
    WOTAPI_FILTER_AMONG_ALL => AccessResult::allowedIfHasPermission($account, 'administer llamas'),
    WOTAPI_FILTER_AMONG_PUBLISHED => AccessResult::allowedIfHasPermission($account, 'view all published llamas'),
    WOTAPI_FILTER_AMONG_OWN => AccessResult::allowedIfHasPermissions($account, ['view own published llamas', 'view own unpublished llamas'], 'AND'),
  ]);
}
 ...
 *
 * Since WOT:API must check field access prior to having a field item list
 ...
 * WOT:API requests where necessary.
 *
 ...
 *   The access result.
 */
function hook_wotapi_entity_field_filter_access(\Drupal\Core\Field\FieldDefinitionInterface $field_definition, \Drupal\Core\Session\AccountInterface $account) {
