 ...
 * WOT:API resource type per entity type bundle. So, for example:
 * - node--article
 * - node--page
 * - node--…
 * - user--user
...
class ResourceTypeRepository implements ResourceTypeRepositoryInterface {
   ...
   */
  public function all() {
    $cached = $this->staticCache->get('wotapi.resource_types', FALSE);
    if ($cached === FALSE) {
      $resource_types = [];
...
//      }
  ...
  public function get($entity_type_id, $bundle) {
    ...
    $cid = "wotapi:resource_type:$entity_type_id:$bundle";
