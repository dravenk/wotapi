...
class FileUpload {
   ...
   * The file uploader.
   *
   * @var \Drupal\wotapi\Controller\TemporaryJsonapiFileFieldUploader
   */
   ...
   * @param \Drupal\wotapi\Controller\TemporaryJsonapiFileFieldUploader $file_uploader
   *   The file uploader.
  ...
  /**
   * Handles WOT:API file upload requests.
   *
  ...
  public function handleFileUploadForExistingResource(Request $request, ResourceType $resource_type, $file_field_name, FieldableEntityInterface $entity) {
    ...
    $route_name = sprintf('wotapi.%s.%s.related', $resource_type->getTypeName(), $file_field_name);
  ...
  public function handleFileUploadForNewResource(Request $request, ResourceType $resource_type, $file_field_name) {
    ...
    // @todo Remove line below in favor of commented line in https://www.drupal.org/project/wotapi/issues/2878463.
    $self_link = new Link(new CacheableMetadata(), Url::fromRoute('wotapi.file--file.individual', ['entity' => $file->uuid()]), ['self']);
    /* $self_link = new Link(new CacheableMetadata(), $this->entity->toUrl('wotapi'), ['self']); */
    ...
    return new ResourceResponse(new WotApiDocumentTopLevel(new ResourceObjectData([$resource_object], 1), new NullIncludedData(), $links), 201, []);
  }
