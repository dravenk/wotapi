...
/**
 * Contains a set of WOT:API Link objects.
 *
...
final class LinkCollection implements \IteratorAggregate {
  ...
  public function __construct(array $links, $context = NULL) {
    ...
    assert(is_null($context) || Inspector::assertAllObjects([$context], WotApiDocumentTopLevel::class, ResourceObject::class));
    ksort($links);
   ...
   *
   * @code
   *   boolean callback(string $key, \Drupal\wotapi\WotApiResource\Link $link, mixed $context))
   * @endcode
   *
   ...
   *
   * @param \Drupal\wotapi\WotApiResource\LinkCollection $a
   *   The first link collection.
   * @param \Drupal\wotapi\WotApiResource\LinkCollection $b
