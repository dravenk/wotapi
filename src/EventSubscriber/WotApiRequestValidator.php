...
/**
 * Request subscriber that validates a WOT:API request.
 *
...
class WotApiRequestValidator implements EventSubscriberInterface {
  ...
  protected function validateQueryParams(Request $request) {
    ...
    foreach (array_keys($request->query->all()) as $query_parameter_name) {
      ...
      if (in_array($query_parameter_name, WotApiSpec::getReservedQueryParameters())) {
        continue;
      }

      if (!WotApiSpec::isValidCustomQueryParameter($query_parameter_name)) {
        $invalid_query_params[] = $query_parameter_name;
      }
    }

    // Drupal uses the `_format` query parameter for Content-Type negotiation.
    // Using it violates the WOT:API spec. Nudge people nicely in the correct
    ...
    if (in_array('_format', $invalid_query_params, TRUE)) {
      ...
      $exception = new CacheableBadRequestHttpException((new CacheableMetadata())->addCacheContexts(['url.query_args:_format']), 'WOT:API does not need that ugly \'_format\' query string! 🤘 Use the URL provided in \'links\' 🙏');
