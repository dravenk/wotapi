 ...
 * 1. It has the @wotapi.serializer service injected instead of @serializer
 * 2. It has the @current_route_match service no longer injected
 * 3. It hardcodes the format to 'api_json'
 * 4. It adds the CacheableNormalization object returned by WOT:API
...
class ResourceResponseSubscriber implements EventSubscriberInterface {
  ...
  protected function renderResponseBody(Request $request, ResourceResponse $response, SerializerInterface $serializer, $format) {
    ...
    if ($data !== NULL) {
      ...
      // @see \Drupal\wotapi\EventSubscriber\DefaultExceptionSubscriber::isWotApiExceptionEvent()
      $context = !$response->isSuccessful() ? [] : static::generateContext($request);
      $wotapi_doc_object = $serializer->normalize($data, $format, $context);
      // Having just normalized the data, we can associate its cacheability with
      // the response object.
      assert($wotapi_doc_object instanceof CacheableNormalization);
      $response->addCacheableDependency($wotapi_doc_object);
      // Finally, encode the normalized data (WOT:API's encoder rasterizes it
      // automatically).
      $response->setContent($serializer->encode($wotapi_doc_object->getNormalization(), $format));
