...
use Drupal\wotapi\Query\EntityCondition;
use Drupal\wotapi\Query\EntityConditionGroup;
...
class TemporaryQueryGuard {
   ...
   *
   * @param \Drupal\wotapi\Query\Filter $filter
  ...
  protected static function secureQuery(QueryInterface $query, $entity_type_id, array $tree, CacheableMetadata $cacheability, $field_prefix = NULL, FieldStorageDefinitionInterface $field_storage_definition = NULL) {
    ...
    foreach ($tree as $specifier => $children) {
      ...
      if (is_null($field_storage_definition)) {
        ...
        // portion. WOT:API will have already validated that the property
        // exists.
   ...
   *   Collects cacheability for the query.
   *
   * @return \Drupal\wotapi\Query\EntityConditionGroup|null
  ...
  protected static function getAccessCondition($entity_type_id, CacheableMetadata $cacheability) {
    ...
    switch ($entity_type_id) {
      ...
      case 'entity_test':
        ...
        // @see \Drupal\wotapi\Tests\Functional\CommentTest::testCollectionFilterAccess()
        $blacklist = \Drupal::state()->get('wotapi__entity_test_filter_access_blacklist', []);
        $cacheability->addCacheTags(['state:wotapi__entity_test_filter_access_blacklist']);
        $specific_conditions = [];
  ...
  /**
   * Gets an access condition for the allowed WOTAPI_FILTER_AMONG_* subsets.
   *
   * If access is allowed for the WOTAPI_FILTER_AMONG_ALL subset, then no
   * conditions are returned. Otherwise, if access is allowed for
   * WOTAPI_FILTER_AMONG_PUBLISHED, WOTAPI_FILTER_AMONG_ENABLED, or
   * WOTAPI_FILTER_AMONG_OWN, then a condition group is returned for the union
  ...
  protected static function getAccessConditionForKnownSubsets(EntityTypeInterface $entity_type, AccountInterface $account, CacheableMetadata $cacheability) {
    // Get the combined access results for each WOTAPI_FILTER_AMONG_* subset.
    ...
    $cacheability->addCacheableDependency($access_results[WOTAPI_FILTER_AMONG_ALL]);
    if ($access_results[WOTAPI_FILTER_AMONG_ALL]->isAllowed()) {
      return NULL;
    }
    ...
    if ($published_field_name) {
      $access_result = $access_results[WOTAPI_FILTER_AMONG_PUBLISHED];
    ...
    if ($status_field_name) {
      $access_result = $access_results[WOTAPI_FILTER_AMONG_ENABLED];
      $cacheability->addCacheableDependency($access_result);
      if ($access_result->isAllowed()) {
    ...
    if ($owner_field_name) {
      $access_result = $access_results[WOTAPI_FILTER_AMONG_OWN];
      $cacheability->addCacheableDependency($access_result);
      if ($access_result->isAllowed()) {
        $cacheability->addCacheContexts(['user']);
  ...
  /**
   * Gets the combined access result for each WOTAPI_FILTER_AMONG_* subset.
   *
   * This invokes hook_wotapi_entity_filter_access() and
   * hook_wotapi_ENTITY_TYPE_filter_access() and combines the results from all
  ...
  protected static function getAccessResultsFromEntityFilterHook(EntityTypeInterface $entity_type, AccountInterface $account) {
    ...
    $combined_access_results = [
      WOTAPI_FILTER_AMONG_ALL => AccessResult::neutral(),
      WOTAPI_FILTER_AMONG_PUBLISHED => AccessResult::neutral(),
      WOTAPI_FILTER_AMONG_ENABLED => AccessResult::neutral(),
      WOTAPI_FILTER_AMONG_OWN => AccessResult::neutral(),
    ];

    // Invoke hook_wotapi_entity_filter_access() and
    // hook_wotapi_ENTITY_TYPE_filter_access() for each module and merge its
    // results with the combined results.
    foreach (['wotapi_entity_filter_access', 'wotapi_' . $entity_type->id() . '_filter_access'] as $hook) {
   ...
   *
   * @return \Drupal\wotapi\Query\EntityConditionGroup
   ...
   *
   * @param \Drupal\wotapi\Query\EntityConditionGroup $group
