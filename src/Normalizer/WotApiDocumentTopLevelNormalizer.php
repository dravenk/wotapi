...
//    ]);
   ...
   *
   * @param \Drupal\wotapi\WotApiResource\OmittedData $omissions
   ...
   *   The normalization context.
   *
   * @return \Drupal\wotapi\Normalizer\Value\CacheableNormalization|\Drupal\wotapi\Normalizer\Value\CacheableOmission
   *   The normalized omissions.
   *
   * @todo: refactor this to use link collections in https://www.drupal.org/project/wotapi/issues/3036279.
   */
  protected function normalizeOmissionsLinks(OmittedData $omissions, $format, array $context = []) {
    ...
    foreach ($normalized_omissions as $omission) {
      ...
      foreach ($omission->getNormalization() as $error) {
        // WOT:API links cannot be arrays and the spec generally favors link
