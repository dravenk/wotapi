...
final class ResourceVersionRouteEnhancer implements EnhancerInterface {
   ...
   *
   * @var \Drupal\wotapi\Revisions\VersionNegotiator
   */
   ...
   *
   * @param \Drupal\wotapi\Revisions\VersionNegotiator $version_negotiator_manager
  ...
  public function enhance(array $defaults, Request $request) {
    if (!Routes::isWotApiRequest($defaults) || !($resource_type = Routes::getResourceTypeNameFromParameters($defaults))) {
      return $defaults;
    }
    ...
    if (!$resource_type->isVersionable()) {
      ...
      if ($has_version_param) {
        ...
        $message = 'WOT:API does not yet support resource versioning for this resource type.';
        $message .= ' For context, see https://www.drupal.org/project/wotapi/issues/2992833#comment-12818258.';
    ...
    if ($defaults[RouteObjectInterface::CONTROLLER_NAME] === Routes::CONTROLLER_SERVICE_NAME . ':getCollection') {
      ...
      if ($request->query->has('filter') && $resource_version_identifier !== $latest_version_identifier) {
        ...
        $message = 'WOT:API does not support filtering on revisions other than the latest version because a secure Drupal core API does not yet exist to do so.';
        throw new CacheableHttpException($cacheability, 501, $message, NULL, []);
      }
    ...
    /** @var \Drupal\wotapi\Revisions\VersionNegotiatorInterface $negotiator */
